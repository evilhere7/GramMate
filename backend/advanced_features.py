    ...
    def check_view_behaviour(self, user_id: str, video_id: str, 
        ...
        if video_duration > 0:
            ...
            if watch_rate < self.suspicious_patterns['unusual_watch_duration']:
                score += 25
                risk_factors.append("Unusually short watch duration (skip suspected)")
            elif watch_rate > 0.99:
                score += 5
                risk_factors.append("Perfect watch completion (suspicious pattern)")
    ...
    def check_engagement_pattern(self, user_id: str, like_count: int, 
        ...
        if view_count > 0:
            ...
            if engagement_rate > self.suspicious_patterns['bot_like_engagement']:
                score += 50
                risk_factors.append("Suspiciously high engagement rate (bot activity)")
...
class PayoutProcessor:
    ...
    def calculate_payout(self, total_earnings: int, platform_fee_rate: float = 0.20) -> int:
...
async def check_engagement_fraud(user_id: str, like_count: int, view_count: int):
    """Check if engagement pattern is suspicious"""
    result = fraud_detector.check_engagement_pattern(user_id, like_count, view_count)
    return result
